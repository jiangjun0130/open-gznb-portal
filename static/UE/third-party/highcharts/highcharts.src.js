...
Highcharts.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// but it can probably be implemented for Firefox 3.5+ on system request. FF3.5+
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// First test the system supplied value, then use the dynamic.
	this.alignOptions = {
...
Axis.prototype = {
	...
	/**
	 * Set the scale based on data min and max, system set min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
	...
	addPlotBandOrLine: function (options, coll) {
		...
		if (obj) { // #2189
			// Add it to the system options for exporting and Axis.update
			if (coll) {
	...
	getOffset: function () {
		...
		// handle automatic or system set offset
...
Tooltip.prototype = {
	...
	/**
	 * In case no system defined formatter is given, this will be used. Note that the context
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		if (outOfBounds) {
			...
			// to the system that the chart is responsive but can't be dragged further.
...
Series.prototype = {
	...
	animate: function (init) {
		...
		// Run the animation
			...
			// which should be available to the system).
	...
	update: function (newOptions, redraw) {
		var chart = this.chart,
			// must use system options when changing type because this.options is merged
